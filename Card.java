...
public class Card
{
    ...
    public void draw(Graphics g, GameState.Phase phase, boolean showHiglight)
	{
	    ...
	    if(currentType == cardType.Cpu_Pick)
	    {
	        if(phase == GameState.Phase.PLAYER1_COMPUTERPICK && showHiglight)
	        {
	            g2.setColor(new Color(4, 196, 36)); //green
	            g2.fillRect(posX - borderSize, posY - borderSize, width + borderSize*2, height + borderSize*2);
	        }
	        else if(phase == GameState.Phase.PLAYER2_COMPUTERPICK && showHiglight)
			{
				g2.setColor(new Color(4, 196, 36)); //green
				g2.fillRect(posX - borderSize, posY - borderSize, width + borderSize*2, height + borderSize*2);
	        }
	    ...
	    if(sprite == spriteList[2])
	    {
			...
			||phase == GameState.Phase.PLAYER1_EXTRAPICK && hovered || phase == GameState.Phase.PLAYER1_COMPUTERPICK && hovered ||
			phase == GameState.Phase.PLAYER2SETUP_POISON && hovered || phase == GameState.Phase.PLAYER2_EXTRAHEART && hovered
			||phase == GameState.Phase.PLAYER2_EXTRAPICK && hovered || phase == GameState.Phase.PLAYER2_COMPUTERPICK && hovered)
			{
